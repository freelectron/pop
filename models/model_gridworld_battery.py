...
def plot_gridworld_value(V, env, policy):
    plt.figure()
    ...
    for y in range(V.shape[0]):
        for x in range(V.shape[1]):
            ...
            if a == 0:
                ...
                dx = -0.3
                dy = 0.0
            plt.arrow(x + 0.5, y + 0.5, dx, dy, alpha=0.7, color="green")

    plt.title(f"Wind west {env.wind_rate_west}; wind south {env.wind_rate_south}")
    plt.colorbar(c)
    plt.gca().invert_yaxis()  # In the array, first row = 0 is on top


class ModelBatteryGridworld(BatteryGridworldEnv):
...
if __name__ == "__main__":
    ...
    # Create true model
    env = BatteryGridworldEnv(shape=GRID_SHAPE, seed=None)
